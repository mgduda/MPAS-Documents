...
\subsection{Scotch}
\label{scotch_build}
Beginning with the MPAS v8.4 release, the {\tt atmosphere} and {\tt init\_atmosphere} cores can be built with
support for the Scotch graph partitioning library. See Section \ref{sec:scotch} for instructions on
how to build MPAS with Scotch support.
...
\section{Linking with the PT-Scotch graph partitioning library}
\label{sec:scotch_build}
Starting with the v8.4.0 release, MPAS also provides an option to enable online graph partitioning using the PT-Scotch library
(\url{https://www.labri.fr/perso/pelegrin/scotch/}). To be able to use this feature, the PT-Scotch library
must first be installed on the system, and then MPAS must be linked with the PT-Scotch library. 

Instructions for building PT-Scotch are available on the Scotch website. The minimum supported PT-Scotch version is v7.0.8. 
As MPAS does not presently support 64-bit integer indices, PT-Scotch build must also be built with 32-bit integers to maintain 
compatibility with MPAS. To be able to use the distributed graph partitioning provided by PT-Scotch, you will need to pass an appropriate MPI
installation path during the Scotch build. Other system prerequisites are Bison and Flex. Note that a more recent version of Flex (>v2.6.4) is recommended.
Scotch documentation also references a requirement for a Bison version > 3.4.

After building and installing PT-Scotch, with a minimum version of at least 7.0.8, the MPAS atmosphere
and init\_atmosphere cores can be linked with Scotch by setting the path to SCOTCH prior to building MPAS.
...
{\tt MPAS was built without OpenACC accelerator support.} \newline
{\tt MPAS was not linked with the MUSICA-Fortran library.} \newline
{\tt \hl{MPAS has been linked with the Scotch graph partitioning library.}} \newline
{\tt Position-dependent code was generated.} \newline
{\tt MPAS was built with .F files.} \newline
{\tt The native timer interface is being used} \newline
{\tt Using the SMIOL library.} \newline
...
{\tt   I/O layer: SMIOL} \newline
{\tt   MUSICA support: no} \newline
{\tt   \hl{SCOTCH support: yes}} \newline
\vspace{12pt}

See Section \ref{sec:scotch_running} for instructions on how to use the PT-Scotch online graph partitioning feature with MPAS.
