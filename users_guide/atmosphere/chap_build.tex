...
must first be installed on the system, and then MPAS must be linked with PT-Scotch library. 

The instructions to build PT-Scotch are available from the Scotch website. The minimum supported PT-Scotch version is v7.0.8. 
As MPAS does not presently support 64-bit integer indices, the PT-Scotch build must also be built with 32-bit integers to maintain 
...
installation path during the Scotch build. Other system pre-requisites are Bison and Flex. Note that a more recent version of Flex (>v2.6.4) is recommended.
Scotch documentation also references the requirement for a Bison version > 3.4.
