...
must first be installed on the system, and then MPAS must be built with PT-Scotch support. 

MPAS does not currently presently support 64-bit integer indices, and to maintain compatibility, the PT-Scotch build must also use 32-bit integers.
The minimum supported PT-Scotch version is v7.0.8.
...
After building MPAS with PT-Scotch, the user can still choose between using a pre-computed graph partition
file or the PT-Scotch online graph partitioning, by setting appropriate values for the      
{\tt config\_block\_decomp\_file\_prefix} namelist option.

If {\tt config\_block\_decomp\_file\_prefix+mpi\_tasks} points to a valid graph partition file that already exists in the run directory, then it is used to proceed with the model run without invoking PT-Scotch.
...
If {\tt config\_block\_decomp\_file\_prefix==''} or {\tt config\_block\_decomp\_file\_prefix+mpi\_tasks} does not match any valid graph partition file in the run directory, then PT-Scotch graph partitioning is invoked. During this process, the generated partition is saved as a graph partition file to the run directory so that it may be reused in the following runs without needing to invoke PT-Scotch again. These run-time choices are summarized in Table~\ref{tab:scotch_runtime_behavior}.

If MPAS has not been built with PT-Scotch, any code paths relating to PT-Scotch will not be taken. And an incorrect specification of {\tt config\_block\_decomp\_file\_prefix+mpi\_tasks} should lead to the model halting. The run-time behavior is summarized in Table~\ref{tab:scotch_runtime_behavior}.

\vspace{12pt}
\begin{longtable}{|p{0.6in}|p{2.7in}|p{2.7in}|}
\hline
\textbf{PT-Scotch support} & \textbf{config\_block\_decomp\_file\_prefix} & \textbf{Run-time behavior} \\ \hline
